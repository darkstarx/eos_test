...
namespace utils
{
	...
	void task_queue::set_max_process_time(const double value)
	{
		ASSERT(value >= 0);
		m_max_process_time = value;
	}
	...
	task_wptr_t task_queue::enqueue_repeatedly(const task_queue::task_f& task, const double interval)
	{
		return enqueue_repeatedly_delayed(task, 0.0, interval);
	}
