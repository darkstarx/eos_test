...
namespace graphics
{
	...
	/** \brief Класс для хранения графических объектов
	 * Объект данного класса не является графическим, но предназначен для хранения графических
	 * объектов. Предоставляет методы для добавления, вставки и удаления графических объектов,
	 * может являться владельцем графических объектов, следит за назначением объектам указателя
	 * на владельца (самого себя) и контролирует, чтобы объекты принадлежали только одному
	 * владельцу.
	 */
